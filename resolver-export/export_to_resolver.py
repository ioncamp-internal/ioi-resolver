...
def fetch_tasks(conn, contest_id):
    with conn.cursor() as cur:
        cur.execute(
            "SELECT id, num, name, title, active_dataset_id, score_mode "
...
def fetch_submissions(conn, contest_id):
    ...
    # score_details 帶著整份 testcase 清單, 存進備份會爆量; 只留每個子題的得分。
    for row in rows:
        row["subtask_scores"] = subtask_scores(row.pop("score_details", None))
    return rows
...
def subtask_scores(score_details):
    """{subtask idx: points} out of a CMS score_details blob, or None.

    None means "this isn't subtask-shaped" -- ScoreTypeSum and friends store a
    flat testcase list, and old backups have no details at all. Callers fall
    back to the submission's own score in that case.
    """
    if isinstance(score_details, (str, bytes)):
        try:
            score_details = json.loads(score_details)
        except (ValueError, TypeError):
            return None
    if not isinstance(score_details, list) or not score_details:
        return None
    out = {}
    for st in score_details:
        if not isinstance(st, dict):
            return None
        if "idx" not in st or "max_score" not in st or st.get("score_fraction") is None:
            return None
        try:
            out[str(st["idx"])] = float(st["score_fraction"]) * float(st["max_score"])
        except (TypeError, ValueError):
            return None
    return out


def parse_subtask_column(value):
    """Read back the compact {idx: points} dict that write_backup stores.

    Empty/absent (a backup taken before the column existed) gives None, which
    makes accumulate_score fall back to the submission's own score.
    """
    if not value:
        return None
...
def accumulate_score(state, score, subtasks, score_mode):
    """Fold one submission into a per-(team, task) accumulator and return the
    task score CMS would show right afterwards.

    `max_subtask` is the IOI 2017 rule: every subtask keeps its best result
    across all submissions and the task score is their sum, so a team can hold
    a score that no single submission of theirs ever achieved (Day5-germany-bone:
    19 then 13 then 44 scores 63, not 44). Emitting the running total rather
    than each submission's raw score also keeps the resolver's reveal honest --
    it takes the max over submissions, so a raw dump would top out at 44 and
    the "reached this score at" tie-break would point at the wrong submission.

    Anything else degrades to `max`, which is what the resolver already assumed.
    """
    if score_mode == SCORE_MODE_MAX_SUBTASK and subtasks:
        best = state.setdefault("subtasks", {})
        for idx, val in subtasks.items():
            if val > best.get(idx, 0.0):
                best[idx] = val
        total = sum(best.values())
    else:
        total = float(score or 0.0)
    total = max(total, state.get("score", 0.0))
    state["score"] = total
    return total
...
def build_contest_json(meta, submissions, indices, max_scores, freeze_minutes, score_modes=None):
    ...
    score_modes = score_modes or {}
    # 每個 (隊, 題) 的累計狀態。必須照時間順序餵進去 -- accumulate_score 是逐筆累積的。
    accum = {}

    seq = 0
    for row in sorted(submissions, key=lambda r: (r["seconds_since_start"], r.get("submission_id") or 0)):
        task_id = row["task_id"]
        ...
        # 只有真的評分完成的提交才算進累計 -- CE/評測失敗的不能推高分數。
        # 累計放在視窗過濾之後: 賽前測試與賽後練習的提交本來就不該進榜。
        scored = (row["compilation_outcome"] != "fail"
                  and row["evaluation_outcome"] == "ok"
                  and row["score"] is not None)
        effective = None
        if scored:
            effective = accumulate_score(
                accum.setdefault((username, task_id), {}),
                row["score"], row.get("subtask_scores"),
                score_modes.get(task_id, SCORE_MODE_MAX),
            )
        verdict = derive_verdict(
            effective, max_scores.get(task_id), row["compilation_outcome"], row["evaluation_outcome"]
        )
        solutions[str(seq)] = {
            "user_id": username,
...
def write_backup(backup_dir, meta, tasks, max_scores, sources, submissions):
    ...
    fieldnames = [
        ...
        "evaluation_outcome", "subtask_scores",
    ]
    with open(csv_path, "w", newline="") as f:
        ...
        for row in submissions:
            writer.writerow({
                ...
                "subtask_scores": (
                    json.dumps(row["subtask_scores"], sort_keys=True)
                    if row.get("subtask_scores") else ""
                ),
            })
...
def load_backup(backup_dir):
    ...
    # 舊備份沒有 score_mode/subtask_scores, 只能照當時的做法退回 max
    score_modes = {t["id"]: t.get("score_mode") or SCORE_MODE_MAX for t in tasks_out}
    ...
    with open(os.path.join(backup_dir, "submissions.csv"), newline="") as f:
        for row in csv.DictReader(f):
            submissions.append({
                ...
                # 備份存的是精簡過的 {idx: 分數}, 不是 CMS 原始的子題清單 --
                # 別拿 subtask_scores() 來讀, 它只認得原始格式。
                "subtask_scores": parse_subtask_column(row.get("subtask_scores")),
            })
    return meta, tasks, max_scores, score_modes, submissions


def do_convert(backup_dir, resolver_dir, freeze_minutes):
    meta, tasks, max_scores, score_modes, submissions = load_backup(backup_dir)
    indices, excluded_tasks = assign_problem_indices(tasks, max_scores)
